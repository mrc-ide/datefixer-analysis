...
orderly_dependency("sim_estim", "latest(parameter:scenario == 'baseline')", c("sim_estim_baseline.rds" = "sim_estim.rds"))
orderly_dependency("sim_estim", "latest(parameter:scenario == 'moderate_sample')", c("sim_estim_moderate_sample.rds" = "sim_estim.rds"))
orderly_dependency("sim_estim", "latest(parameter:scenario == 'very_large_sample')", c("sim_estim_very_large_sample.rds" = "sim_estim.rds"))
...
sim_estim_moderate_sample   <- readRDS("sim_estim_moderate_sample.rds")
sim_estim_baseline          <- readRDS("sim_estim_baseline.rds")
sim_estim_very_large_sample <- readRDS("sim_estim_very_large_sample.rds")

sim_estim <- c(sim_estim_moderate_sample, sim_estim_baseline, sim_estim_very_large_sample)
...
agg_sims_summary <- summary_with_labels %>%
  mutate(
    scenario = factor(scenario, levels =
                        c("very_small_sample", "small_sample", "moderate_sample",
...
ggplot(sample_size_scenarios,
  ...
  geom_point(size = 2) +
  geom_point(aes(y = mu_50cov, group = group, color = delay)) +
...
# Bias (mean +/- SD) for mu
ggplot(agg_sims_summary, aes(x = delay, y = mu_mean_bias, color = group)) +
  geom_point() +
...
# Bias (mean +/- SD) for cv
ggplot(agg_sims_summary, aes(x = delay, y = cv_mean_bias, color = group)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  facet_wrap(~ group + scenario, ncol = 3, scales = "free_x") +
